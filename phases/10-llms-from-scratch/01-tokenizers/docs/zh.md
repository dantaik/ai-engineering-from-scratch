...
- **成本 B：序列长度（持续性，按请求付费）。**词表越大压缩越狠：同一段英文，32K 词表切 100 个 token，128K 词表可能只要 70 个——生成时少 30% 的前向传播。还有一笔隐藏放大：**注意力是 O(n²) 的**——注意这里的 n 是**序列里的 token 数**，不是词表大小：自注意力里每个 token 要和序列里所有 token 算一遍相关性，n 个 token 就是 n² 对。词表大小 V 从不直接出现在注意力公式里，它只通过"同一段文本切出多少个 token"**间接**改变 n。序列长 43%（100/70），注意力计算量就翻倍，压缩收益被平方放大。
...
把本节的账收拢成一张表。表里写的是**词表变大**的效果，词表变小就把每个格子反过来读：

| 词表变大 → | 训练阶段 | 推理阶段 |
|---|---|---|
| **赚在哪** | 同样语料切出的 token 更少，同样算力能"读"更多文本；序列更短，O(n²) 的注意力更便宜，同样显存能塞更大的 batch | 生成同样内容走更少的步（每个 token 一次完整前向传播），延迟更低、吞吐更高；KV cache 随序列缩短而变小；同样的 token 窗口装更多内容；按 token 计费的账单更薄 |
| **付在哪** | 嵌入矩阵和输出投影随词表线性膨胀，参数和显存上涨；每步输出层要对更大的词表算 logits 和交叉熵；长尾 token 的出现次数被摊薄，欠训练风险上升（glitch token 的温床），需要更多、更均衡的数据 | 两头的大矩阵常驻显存（端侧部署和小模型最痛）；每生成一步都要在更大的词表上算 logits、做采样 |

一条帮助记忆的主线：**词表大小重新分配的是"要走多少步 × 每步多贵"这笔乘积。**大词表让步数变少、每步变贵——但省下的每一步都是整条前向传播（模型所有层少跑一遍），付出的只是输出那一层变宽。所以模型越深越大，这笔交换越划算；反过来，模型越小（输出层占比越高）、部署越靠端侧（显存越金贵）、或者你最在乎罕见词的碎片学得扎实，小词表越有吸引力。
...
### 另一个活例子：DeepSeek 的分词器

问"DeepSeek 在分词上有什么不一样"，答案分两半。

**路线上并不特立独行**：DeepSeek 从第一代起就是自研训练的 byte-level BPE（用 Hugging Face 的 tokenizers 库实现），和 GPT、Llama 3 同一条技术路线。第一代词表 10 万常规 token + 15 个特殊 token（模型端补齐到 102,400），V2 沿用，V3 扩到 128K；R1 推理系列建在 V3 底座上，用同一套分词器。数字处理上第一代沿了 Llama 的老路——逐位切分。

**真正不一样的是两件事：**

1. **词表在中文占比很高的多语言语料上训练。**同一段中文，DeepSeek 的分词器切出的 token 数明显少于以英语为主的 cl100k 系词表——常用双字词、成语能长成整块 token。这是多语言税最直接的反面教材：**词表谁来训，谁的语言坐头等舱。**
2. **V3 干了一件公开报告里少见的细活。**新的预切分器允许"标点 + 换行"合成组合 token（多行文本压缩更狠），但这引入了 token 边界偏差：当多行 prompt 结尾恰好没带换行时，模型落在训练中罕见的边界状态上，few-shot 评测尤其受影响。DeepSeek 的解法是**训练时随机把一部分组合 token 拆开**，让模型对两种切法都见过世面。你应该觉得眼熟——这和第 6 节 Unigram 的 subword regularization 是同一个思想：用切分的随机性给模型脱敏。
